module EmbeddedTerraformRepositoryHelper::TextualSummary
  ...
  def textual_group_relationships
    TextualGroup.new(_("Relationships"), %i[provider templates])
  end
  ...
  def textual_templates
    h = {:label => _('Templates'), :value => @record.total_payloads}
    if @record.total_payloads.positive? && role_allows?(:feature => 'embedded_configuration_script_payload_view')
      h.update(:link  => url_for_only_path(:action => 'show', :id => @record, :display => 'templates'),
